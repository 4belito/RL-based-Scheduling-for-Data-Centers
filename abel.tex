...
\begin{frame}{Defining the Problem as an MDP}
  ...
  \begin{block}{Markov Decision Process}
    We frame scheduling as an MDP and learn the job selection policy with RL:
  ...
  \begin{tabular}{c l c}
    ...
    \midrule
    $N$              & total nodes in the cluster     & $64$ \\
    $K$              & max visible queue length       & $10$ \\
    $\Delta t$       & seconds advanced per step      & $60$ \\
    $T$              & episode length (seconds)       & $86\,400$ (24\,h) \\
    $H = T/\Delta t$ & decision horizon (steps)       & $1\,440$ \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
